...
def create_table_if_not_exists(conn):
    ...
    CREATE TABLE IF NOT EXISTS trails (
        ...
        geometry TEXT, -- Using TEXT for WKT for MVP, can be upgraded to PostGIS Geometry
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(query)
        conn.commit()
        logger.info("Checked/Created 'trails' table successfully.")
    except Exception as e:
        conn.rollback()
        logger.error(f"Error creating table: {e}")
        raise

def import_trails(csv_filepath):
        ...
        with open(csv_filepath, mode='r', encoding='utf-8') as file:
            ...
            with conn.cursor() as cur:
                for row in reader:
                    ...
                    fort = cur.fetchone()
                    if not fort:
                        logger.warning(f"Fort '{row['fort_name']}' not found for trail '{row['name']}'. Skipping.")
                        continue
                    fort_id = fort[0]
                ...
                conn.commit()
                logger.info(f"Import complete. Inserted: {inserted_count} trails.")
...
if __name__ == "__main__":
    ...
    csv_path = os.path.join(script_dir, '..', 'data', 'trails.csv')
    import_trails(csv_path)
